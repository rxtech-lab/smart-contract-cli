...
func (s *GeneratorTestSuite) TestGenerateRoutesFile_SingleRoute() {
	...
	s.Contains(code, `{Path: "/", Component: NewPage()}`)
}
...
func (s *GeneratorTestSuite) TestGenerateRoutesFile_MultipleRoutes() {
	...
	s.Contains(code, `{Path: "/", Component: NewPage()}`)
	s.Contains(code, `{Path: "/users", Component: users_page.NewPage()}`)
	s.Contains(code, `{Path: "/users/:id", Component: users_id_page.NewPage()}`)
}
